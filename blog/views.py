...
def index(request):
    ...
    comments = Comment.objects.filter(for_zawadi=True).filter(
        is_published=True).order_by('-point')[:10]
...
def get_ssubject(request, slug):
    ...
    arts = SubSubjects.objects.get(
        pk=pk).articles.all().order_by('-created_at')
    arts, can_con = get_queryset_from_paginator(arts=arts, number=8, p=p)
    comments = Comment.objects.filter(for_zawadi=True).filter(
        is_published=True).order_by('-point')[:10]
    return render(request, 'blog/index.html', {
...
def set_things(request):
    ...
    if int(sub):
        ...
        dem = ClientDemand.objects.create(
            client=client, category=sub.box, subs=sub, emergency=emer, quart=client.user.quart, slug=unique_slug)
    if activity != 'norm':
        if activity == 'comment':
            Comment.objects.create(article=Articles.objects.get(
                pk=int(article)), user=client.user, text=comment)
        else:
            Comment.objects.create(
                user=client.user, text=message, for_zawadi=True)
    return Response({
        'done': True,
        'result': 'sub' if sub else 'com'
    })
